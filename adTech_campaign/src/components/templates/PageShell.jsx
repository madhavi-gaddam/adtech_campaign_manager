...
import { LayoutDashboard, Megaphone, Moon, PlusCircle, ShieldCheck, Sun, UserCircle2 } from 'lucide-react'
    ...
    <div className={`${isDashboard ? 'lg:h-screen lg:overflow-hidden' : 'min-h-screen'} overflow-x-clip bg-gray-100`}>
      ...
      <div className={`flex min-w-0 ${isDashboard ? 'lg:h-screen' : 'min-h-screen'}`}>
        <aside className="sticky top-0 hidden h-screen w-64 shrink-0 border-r border-gray-200 bg-white px-4 py-5 lg:block">
          ...
          <div className="absolute bottom-5 left-4 right-4 flex items-center justify-center gap-2">
            <button type="button" onClick={() => setIsLogoutDialogOpen(true)} className="inline-flex h-10 items-center justify-center rounded-md border border-slate-300 px-3 text-sm font-bold text-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:border-slate-600 dark:text-slate-100">Logout</button>
            <button
              type="button"
              onClick={() => setTheme(isDark ? 'light' : 'dark')}
              aria-label={`Switch to ${isDark ? 'light' : 'dark'} theme`}
      ...
      {isLogoutDialogOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 px-4">
          <div role="dialog" aria-modal="true" aria-labelledby="logout-dialog-title" className="w-full max-w-md rounded-lg bg-white p-4 shadow-xl sm:p-6">
            <h2 id="logout-dialog-title" className="text-lg font-bold text-gray-900">Logout?</h2>
            <p className="mt-2 text-sm text-gray-600">Are you sure you want to logout?</p>
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setIsLogoutDialogOpen(false)} className="inline-flex h-10 items-center justify-center rounded-md border border-slate-300 px-4 text-sm font-bold text-gray-700">Cancel</button>
              <button type="button" onClick={signOut} className="inline-flex h-10 items-center justify-center rounded-md bg-rose-600 px-4 text-sm font-bold text-white hover:bg-rose-700">Logout</button>
            </div>
          </div>
        </div>
      )}
